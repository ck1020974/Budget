...
function generateGroupFeedback(resultKey, onlyIgnored = false) {
  ...
  if (resultKey === '高票當選_高') {
    ...
    if (caredGroups.length === 0) {
      return `<p>在你的任內，似乎沒有推動任何具體的福利政策，所有族群都未得到特別的關注。</p>`;
    }
    return feedbackHtml;
  }
  ...
  if (resultKey === '敗選退場_低') {
    ...
    if (ignoredGroups.length === 0) {
      return `<div class="feedback-card"><似乎沒有推動任何具體的福利政策，所有族群都未得到特別的關注。</p></div>`;
    }
    return feedbackHtml;
  }
  ...
  if (feedbackHtml.split('feedback-card').length - 1 < 3) {
    const existingCount = feedbackHtml.split('feedback-card').length - 1;
    const needed = 3 - existingCount;
    const allMessages = Object.entries(groupFeedbackTexts)
      .flatMap(([group, obj]) => [obj['被關注'], obj['被忽略']])
      .filter(msg => msg && !feedbackHtml.includes(msg));
  
    for (let i = 0; i < needed && i < allMessages.length; i++) {
      feedbackHtml += `<div class="feedback-card">${allMessages[i]}</div>`;
    }
  }
  ...
  let mostCaredFor = [];
  let leastCaredFor = [];
  const isWin = resultKey === '高票當選' || resultKey === '驚險連任';
