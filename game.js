...
function startNewRound() {
  ...
  if (roundCount === 5 || roundCount === 10 || roundCount === 15) {
    const addBudget = 300000000000; // 3000 億
    totalBudget += addBudget;
    alert(`第${roundCount}回合結束，追加預算3000億！`);
    updateBudgetDisplay();
  }
  if (roundCount >= MAX_ROUNDS) {
    clearInterval(countdownInterval);  // 停止倒數計時器
    document.getElementById('round-container').innerHTML = '<p style="text-align:center; font-size:1.2rem;">🎉 已達 20 回合，遊戲結束！</p>';
...
const groupFeedbackTexts = {
  '嬰幼兒': {
    '被關注': '政府積極推動托育與支持政策，家長普遍感受到育兒負擔減輕，支持度大幅提升。',
  ...
  '高齡者': {
    ...
    '被忽略': '充高齡者在就醫與生活上遭遇困境，照護資源不足，讓晚年生活充滿不安與焦慮。'
  },
  '身心障礙者': {
...
function generateGroupFeedback(resultKey, onlyIgnored = false) {
  ...
  if (onlyIgnored) {
    ...
    if (ignoredGroups.length === 0) {
      return `<div class="feedback-card"><p>在你的任內，似乎沒有推動任何具體的福利政策，所有族群都未得到特別的關注。</p></div>`;
    }
    return feedbackHtml;
  }
